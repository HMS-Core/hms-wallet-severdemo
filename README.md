# HUAWEI Wallet Kit Server Demo (PHP Version)
### Table of Contents
...
PHP 7.4 or above is recommended.
...
Before running the Demo project, you need to set the following configuration values in the "test\config\config.ini" file: "gw.appid", "gw.appid.secret", "gw.tokenUrl", "walletServerBaseUrl", "servicePrivateKey", and "walletWebsiteBaseUrl"
...
Input parameters of models and instances are passed by JSON files in the Demo project. You can generate your own data by modifying the JSON files in the "test\config\data" folder.<br>
...
Install dependencies in the composer.json file.
...
## License
HUAWEI Wallet Kit Server Demo (PHP Version) is licensed under the [Apache License, version 2.0](http://www.apache.org/licenses/LICENSE-2.0).
