...
public final class Base64 {
    ...
    private static byte[] checkCharacters(char[] base64Data, char d1, char d2, int idx, int encodedIndex, int dataIndex,
        ...
        if (isNotD3OrD4(d3, d4)) {
            if (isPadD3AndD4(d3, d4)) {
                if ((b2 & 0xf) != 0) {
                    // The last 4 bits should all be zero.
                    return new byte[0];
                }
                ...
                b3 = BASE64_ALPHABET[d3];
                if ((b3 & 0x3) != 0) {
                    // The last 2 bits should be zero.
                    return new byte[0];
                }
    ...
    /**
     * Remove whitespaces from a char array.
     *
     * @param data the char array with whitespaces.
     * @return the array's length after removing whitespaces.
     */
    private static int removeWhiteSpace(char[] data) {
        ...
        // Count characters that are not whitespaces.
        int newSize = 0;
        int len = data.length;
        for (int i = 0; i < len; i++) {
